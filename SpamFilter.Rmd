...
#### Training Data
```{R Evaluate Training Performance, fig.cap="Confusion Matrix and Performance Metrics of Training Data"}
...
#### Test Data
```{R Evaluate Testing Performance, fig.cap="Confusion Matrix and Performance Metrics of Test Data"}
predictCM(rpartFit1, test, "class")
pred1 <- prediction(predict(rpartFit1, type = "prob")[, 2], train$isSpam)
...
In reference to the above plots and confusion matrix, the overall accuracy of the model is 89.51%, with a confidence of 88.03% to 90.87%. The ROC curve demonstrates the true positive rate peaks when the false positive rate is around 25%, leading to the decrease in overall accuracy. McNemar's test is not statistically significant, indicating that there is likely not randomness in the difference between spam and non-spam. The No Information Rate and associated p-value overwhelmingly suggests that the accuracy rate is larger than the majority class (in this case, the negative class). This suggests it might be prudent to cross validate in future tests.

### Variable Importance
A plot to visualize the feature variable importances is provided below in Figure 11. The splits in the decision tree were performed on the information gain index which is a measure of reduction in entropy or uncertainty in the data. The classification tree model aims to reduce the entropy (a measure of impurity) in the target variable (isSpam) by splitting the emails into subpopulations. There is roughly 30% impurity in the isSpam target variable since the original strata noted in the EDA was 70% non-spam and 30% spam.

The decision tree splits these emails into partitions to reduce the impurity of the population based on the explanatory variables. The greater the reduction in this impurity following a split on an explanatory variable, the more information gained about the target variable from this explanatory variable. This provides a measure of which explanatory variables are the most useful in discriminating between classes of our target variable. 

From the classification tree trained, the 3 most important features in discriminating between spam and non-spam emails are the number of forwards, the percentage of capitals among all letters in the message body (perCaps), and if “Re:” appears at the start of the subject (isRe). The information gain measure of these top 3 variables are colored below in green within Figure 11.

<br>

```{R Variable Importance Plot, fig.align="center", fig.height=7, fid.width=7}
...
## Conclusion
The existing outliers were kept when training the classification tree model since they did not appear to be errors. The outlier for the numDlr variable was also kept since it was not an important feeature that heavily influenced differentiation between spam and non-spam emails. A separate model was also trained with the removal of the numLines variable, but did not show noticable improvements over the tree model with all variables included (provided in the Appendix). The decision was made to train the model using all features rather than exlcuding numLines.

The variables "forwards", "perCaps", and "isRe" provided the most information gain following a partition. However, it is interesting to note that the decision tree plotted did not have any partition splits on the "isRe" variable despite its large information gain value. The other two variables "forwards" and "perCaps" were shown to be important splits in the decision tree, occuring as the first two partition splits in the tree.

The classification tree generalized well to the test data with slightly lower performance than the training data and did not present with issues of overfitting. The classification accuracy of 89.6% indicates that the model has good overall performance with 85.1% sensitivity and 91.6% specificity. However, the model has a comparatively low precision (positive predictive value) of 81.5% as a result of fewer spam messages in the data compared to non-spam messages. 

Suggestions to further improve this study are to employ a stratified k-fold cross validation scheme on the training data split to limit model bias since only a stratified train-test split was used in this analysis. Furthermore, an ensemble learning method such as random forests on classification trees could be implemented to improve model performance, particularly to address the lower precision metric. Additional trees in an ensemble model should aim to include the "isRe" feature since it was one of top 3 most important variables based on information gain.
